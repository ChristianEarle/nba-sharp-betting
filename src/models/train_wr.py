"""WR breakout modeling & validation (Phase 4, WR only).

Pipeline (``run_full_pipeline``): tune classifier head (LGBM + XGB + L2
logistic, per ``configs/model_wr.yaml``) on the four expanding-window
...
Modeling universe
------------------
``in_training_pool == 1`` rows of ``features_wr.parquet`` joined to
...
- **Logistic head** gets a curated ~15-column subset
  (``configs/model_wr.yaml``'s ``logistic_feature_subset`` + the same
...
``tests/test_models.py::test_determinism``.
...
CONFIG_PATH = REPO_ROOT / "configs" / "model_wr.yaml"

FEATURES_WR_PATH = PROCESSED_DIR / "features_wr.parquet"
LABELS_PATH = PROCESSED_DIR / "labels.parquet"

REPORT_PATH = OUTPUTS_DIR / "model_wr_report.md"
METRICS_JSON_PATH = OUTPUTS_DIR / "model_wr_metrics.json"
ARTIFACT_PATH = MODELS_DIR / "wr_model_bundle.joblib"
...
def load_modeling_frame(
    ...
    """features_wr.parquet joined to labels.parquet, restricted to in_training_pool==1.
    ...
    this module is the modeling layer.
    """
    if cfg is None:
        cfg = load_config()
...
def tune_logistic(
    ...
    """Small fixed grid over C (not Optuna — see configs/model_wr.yaml), same fold-mean-PR-AUC objective."""
...
def run_full_pipeline(
    cfg: dict | None = None, classifier_trials: int | None = None, regression_trials: int | None = None, seed: int | None = None
) -> dict:
    ...
    save_artifacts(frozen, seed, cfg)
...
def generate_report(result: dict, pytest_summary: str | None = None) -> str:
    ...
    lines = []
    lines.append("# BreakoutLab -- WR Model Report (Phase 4)")
    lines.append("")
    lines.append(verdict)
    lines.append("")
    lines.append(
        "~2,100 rows in the WR feature table, ~1,200-1,460 in the training pool, and a 3-6% "
        "positive rate per season (see per-fold `n_pos` below): PR-AUC is expected to be noisy "
        "fold-to-fold. Fold-level numbers are reported alongside every pooled mean for exactly "
        "that reason -- read the spread, not just the mean."
    )
    lines.append("")
    ...
    lines.append(
        ...
        "(`configs/model_wr.yaml`'s `search_space.logistic.C`), not Optuna -- the brief scopes "
        "Optuna tuning to LGBM/XGB only (\"Optuna per model type (LGBM/XGB)\")."
    )
    lines.append(
...
def write_outputs(result: dict, pytest_summary: str | None = None) -> None:
    ...
    METRICS_JSON_PATH.write_text(json.dumps(metrics, indent=2, default=str))
...
def main() -> int:
    print("WR model training | Phase 4 (full config: see configs/model_wr.yaml)")
    ...
    write_outputs(result, pytest_summary=pytest_summary)
